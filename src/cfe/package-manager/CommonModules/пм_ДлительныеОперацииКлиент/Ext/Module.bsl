...
Процедура ОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении = Неопределено, 
	...
	Если РасширенныеПараметры.ВыводитьОкноОжидания Тогда
		...
		ОткрытьФорму("ОбщаяФорма.пм_ДлительнаяОперация", РасширенныеПараметры, 
...
//		СерверныеОповещенияКлиент.ПодключитьОбработчикПроверкиПолученияСерверныхОповещений();
		ПодключитьОбработчикОжидания("пм_КонтрольДлительныхОпераций", РасширенныеПараметры.ТекущийИнтервал, Истина);
...
Функция АктивныеДлительныеОперации() Экспорт
	...
	Если пм_ПараметрыПриложения[ИмяПараметра] = Неопределено Тогда
		...
		пм_ПараметрыПриложения.Вставить(ИмяПараметра, Операции);
	КонецЕсли;
	
	Возврат пм_ПараметрыПриложения[ИмяПараметра];

КонецФункции

Процедура ПроверитьПараметрыОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении, Знач ПараметрыОжидания)
	
	пм_ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ДлительныеОперацииКлиент.ОжидатьЗавершение",
	...
	Если ПараметрыОжидания <> Неопределено Тогда
		...
		СообщениеПроверки = пм_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Параметр %1 должен быть больше или равен 1'"), "ПараметрыОжидания.Интервал");
		
		пм_ОбщегоНазначенияКлиентСервер.Проверить(ПараметрыОжидания.Интервал = 0 Или ПараметрыОжидания.Интервал >= 1,
		...
		пм_ОбщегоНазначенияКлиентСервер.Проверить(Не (ПараметрыОжидания.ОповещениеОПрогрессеВыполнения <> Неопределено И ПараметрыОжидания.ВыводитьОкноОжидания), 
			СообщениеПроверки, "ДлительныеОперацииКлиент.ОжидатьЗавершение");
